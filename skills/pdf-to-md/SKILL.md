...
| PDF（扫描 / 图像） | `ocr_extract.py` (PaddleOCR 异步 API) | 自动识别后跳过快速路径 |
...
OCR 输出结构：
- `per_page/`：逐页独立 MD（仅多页时生成）
- `merged/`：全文合并 MD，适合直接导入知识库
- `pymupdf_raw/`：pymupdf 并行提取的原文（仅 PDF 文件生成）
...
- `PADDLEOCR_TOKEN`：云端 OCR 认证 token（仅 Step 2 需要）
