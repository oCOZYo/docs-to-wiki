...
所以会走 pdf-to-md 的快速路径——零 OCR 消耗，秒级完成。
...
- `~/.venvs/general/bin/python`：含 `pymupdf`（pdf-to-md 依赖）
...
- **视觉布局简化**：本 Skill 走 PDF 文字路径，PPTX 中通过空间布局表达的信息
  ...
  会丢失部分视觉关系。如果需要精确还原视觉结构，未来可在 pdf-to-md 端
  增加"视觉密集页"识别 + Vision 整页描述的分流逻辑（不在本 Skill 范围内）
